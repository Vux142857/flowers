...
const UserFilter: React.FC<UserFilterProps> = ({ counts }) => {
  const router = useRouter();
  const [name, setName] = useState<string>("");
  const [status, setStatus] = useState<string>("");
  const [query,] = useState<IUserFilter>({
    status: "",
  })
  ...
  const handleSearch = useCallback(() => {
    router.push(`/admin/${Schemas.CUSTOMER}?action=${QueryAction.SEARCH}&query=${name}`)
  }, [name, router])

  const handleFilter = useCallback(() => {
    router.push(`/admin/${Schemas.CUSTOMER}?action=${QueryAction.FILTER}&status=${status}`)
  }, [router, status])
  ...
  return (
    <div className="flex flex-col border-2 p-4 rounded gap-4">
      ...
      <div className="flex gap-4 mb-4">
        ...
        >
          <option value="">Status of customer</option>
          {
