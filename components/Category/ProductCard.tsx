...
const ProductCard: React.FC<ProductCardProps> = (props) => {
  return (
    <Link href={`/product/${props.id}`} className={`w-full flex items-center justify-center relative border-[0.5px] ${props.isMain ? 'border-black' : 'border-ligh'} overflow-hidden`}>
      <Image src={props.imageUrl} alt={props.name} className="cursor-pointer w-full h-full object-cover transition-transform duration-300 hover:scale-125" />
